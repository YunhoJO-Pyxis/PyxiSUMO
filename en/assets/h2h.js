...
(function () {
  ...
  function Picker(slot) {
    ...
    function render() {
      ...
      list.textContent = '';
      if (!hits.length) {
        var none = el('li', 'h2h-none', "No matching rikishi.");
  ...
  function show(a, b, remember) {
    ...
    fetch(D.src + encodeURIComponent(a) + '.json')
      ...
      .catch(function () { if (mine === seq) { say("Couldn't load the record. Please try again in a moment."); } });
  }
  ...
  function draw(pa, pb, bouts) {
    ...
    var notes = [];
    if (done.length) {
      notes.push("{n} bouts in total".replace('{n}', done.length));
      notes.push("{a} – {b}"
        .replace('{a}', D.basho[done[done.length - 1][0]] || done[done.length - 1][0])
        .replace('{b}', D.basho[done[0][0]] || done[0][0]));
      if (fusen) { notes.push("incl. {n} by default".replace('{n}', fusen)); }
    }
    if (next) {
      notes.push("Scheduled for {d}".replace('{d}', "Day {d}".replace('{d}', next[1])));
    }
    if (notes.length) { out.appendChild(el('p', 'h2h-sum', notes.join(' \u00b7 '))); }

    if (!done.length && !next) {
      out.appendChild(el('p', 'search-empty', "They have not met yet."));
      return;
    }
    ...
    bouts.forEach(function (x) {
      ...
      row.appendChild(el('td', 'num', "Day {d}".replace('{d}', x[1])));
      function cell(rv, win) {
        var td = el('td', 'h2h-cell');
        td.appendChild(el('span', 'h2h-r', rankOf(rv)));
        if (win === true) { td.appendChild(el('span', 'bt-win', "W")); }
        if (win === false) { td.appendChild(el('span', 'bt-lose', "L")); }
        return td;
      }
      var r = x[2];
      ...
      if (r === null) {
        kim.appendChild(el('span', 'bt-pending',
          x[0] === D.cur ? "Scheduled" : "No result"));
      } else if (x[3]) {
        kim.textContent = "Default";
      } else {
  ...
  form.addEventListener('submit', function (ev) {
    ...
    var a = A.resolve();
    var b = B.resolve();
    if (!a || !b) { say("Please pick a rikishi in both boxes."); return; }
    if (a === b) { say("Please pick two different rikishi."); return; }
    show(a, b, true);
  });
