...
(function () {
  ...
  function Picker(slot) {
    ...
    function render() {
      ...
      list.textContent = '';
      if (!hits.length) {
        var none = el('li', 'h2h-none', "맞는 선수가 없습니다.");
  ...
  function show(a, b, remember) {
    ...
    fetch(D.src + encodeURIComponent(a) + '.json')
      ...
      .catch(function () { if (mine === seq) { say("기록을 불러오지 못했습니다. 잠시 뒤 다시 눌러 주세요."); } });
  }
  ...
  function draw(pa, pb, bouts) {
    ...
    var notes = [];
    if (done.length) {
      notes.push("총 {n}번 맞붙음".replace('{n}', done.length));
      notes.push("{a} ~ {b}"
        .replace('{a}', D.basho[done[done.length - 1][0]] || done[done.length - 1][0])
        .replace('{b}', D.basho[done[0][0]] || done[0][0]));
      if (fusen) { notes.push("부전 {n}번 포함".replace('{n}', fusen)); }
    }
    if (next) {
      notes.push("{d} 대전 예정".replace('{d}', "{d}일째".replace('{d}', next[1])));
    }
    ...
    var thead = el('thead');
    var tr = el('tr');
    ["대회", "일차", pa.d, pb.d, "결정수"].forEach(function (h) {
    ...
    bouts.forEach(function (x) {
      ...
      row.appendChild(el('td', 'num', "{d}일째".replace('{d}', x[1])));
      function cell(rv, win) {
        var td = el('td', 'h2h-cell');
        td.appendChild(el('span', 'h2h-r', rankOf(rv)));
        if (win === true) { td.appendChild(el('span', 'bt-win', "승")); }
        if (win === false) { td.appendChild(el('span', 'bt-lose', "패")); }
        return td;
      }
      var r = x[2];
  ...
  form.addEventListener('submit', function (ev) {
    ...
    var a = A.resolve();
    var b = B.resolve();
    if (!a || !b) { say("두 칸 모두 목록에서 선수를 골라 주세요."); return; }
    if (a === b) { say("서로 다른 두 선수를 골라 주세요."); return; }
    show(a, b, true);
  });
